...
**Status: accepted (#17); built under #18.** This document says what the ontology is for, who uses it, how an
entry comes to exist, and what an edition promises the tools that pin it. The last table says
what the corpus does today.
...
| Statuses | `verified`, `partial`, `unverified`, `interpretation`, as now | `defined` when the ontology sets the meaning, with a `rationale`; `verified` when a primary source states it exactly as the entry does |
...
It holds from the first edition after v0.6.0, and `tools/changelog.py --promise` checks the parts
the data can show.
...
before re-pinning: [`CHANGELOG.md`](../CHANGELOG.md), generated by `tools/changelog.py`.
...
| | Where |
|---|---|
| Rules primary-sourced, executed, edition-tracked | `constraints/`, `tools/validate.py`, `tools/check_editions.py` |
| The `defined` status, refused on a rule or a unit, requiring a `rationale` | the schemas, `tools/validate.py`, `tools/test_validate_definitions.py` |
| Definitions re-filed as `defined` | `hull-length`, `beam-at-main-step`, `maximum-beam`, `vertical-tail-area` (verification log, Round 16) |
| A bounded search for a definition | `CONTRIBUTING.md`; recorded in each entry's `checked_against` |
| `superseded_by`, resolving to a current entry of the same layer | the schemas, `tools/validate.py` |
| The edition promise and a changelog | `CONTRIBUTING.md`, `tools/changelog.py`, `tools/test_changelog.py`, `CHANGELOG.md` |
