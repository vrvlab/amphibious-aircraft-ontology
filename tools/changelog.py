...
promise), which applies from the first edition after v0.6.0:
  ...
  * an id is not removed unless the edition before already marked it
    `superseded_by`;
...
def diff(old: dict, new: dict) -> dict[str, dict[str, list]]:
    out = {}
    for layer in LAYERS:
...
def main() -> int:
    ...
    last = ts[-1]
    d = diff(load(last), load(WORKTREE))
